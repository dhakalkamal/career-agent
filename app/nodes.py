...
Hybrid architecture with LangChain message integration.
"""
...
def parse_json_response(content: str) -> Any:
    ...
    try:
        ...
        logger.debug("Failed JSON content: %s", content)
        return {}
...
def router_node(state: CareerCoachState) -> CareerCoachState:
    ...
    logger.info("Router: questions_asked=%d", questions_asked)
...
def discovery_node(state: CareerCoachState) -> CareerCoachState:
    ...
    if current_focus is None:
        logger.info("Discovery: no focus set, skipping question")
    ...
    try:
        ...
        logger.error("Discovery node error: %s", e)
        return {
            **state,
...
def synthesis_node(state: CareerCoachState) -> CareerCoachState:
    ...
    try:
        ...
        logger.error("Synthesis node error: %s", e)
        return {
            **state,
            "user_profile": {"interests": [], "skills": [], "work_style": [], "constraints": []},
            "insights": [],
...
def matching_node(state: CareerCoachState) -> CareerCoachState:
    ...
    try:
        ...
        logger.error("Matching node error: %s", e)
        return {
            **state,
            "career_matches": [],
            "error_count": state.get("error_count", 0) + 1,
...
def action_node(state: CareerCoachState) -> CareerCoachState:
    ...
    try:
        ...
        logger.error("Action node error: %s", e)
        return {
            **state,
